...
- ACPX owns the built-in package range so fresh launches use the repository-selected stable adapter line without requiring a global adapter install. The npm distribution includes that adapter; the Homebrew distribution depends on Node.js and resolves it through `npx`.
