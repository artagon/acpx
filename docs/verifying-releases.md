...
gh attestation verify "$(npm pack "acpx@$version" --silent)" --repo openclaw/acpx
```
...
closure comes from the bundled ACP adapters. It is generated by a
checksum-pinned [Syft](https://github.com/anchore/syft) release binary, scanning
the pruned lockfile from a production-only pnpm deploy. The workflow verifies
that the built-in adapters are present and representative dev-only packages are
